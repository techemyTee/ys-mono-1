...
namespace ysservices.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("ysservices") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "ysservices", DisplayName = "ysservices API", Resources = { "ysservices" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var consoleAndAngularClientId = configurationSection["ysservices_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["ysservices_App:RootUrl"]?.TrimEnd('/');
        ...
        //Mobile Client
        var mobileClientId = configurationSection["ysservices_Mobile:ClientId"];
        if (!mobileClientId.IsNullOrWhiteSpace())
        {
            var mobileClientRootUrl = configurationSection["ysservices_Mobile:RootUrl"]?.Replace("_", "-");
            await CreateApplicationAsync(
                name: mobileClientId!,
                type: OpenIddictConstants.ClientTypes.Public,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
        ...
        // Web Public Client
        var webPublicClientId = configurationSection["ysservices_Web_Public:ClientId"];
        if (!webPublicClientId.IsNullOrWhiteSpace())
        {
            var webPublicRootUrl = configurationSection["ysservices_Web_Public:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["ysservices_Web_Public:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["ysservices_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["ysservices_Swagger:RootUrl"]?.TrimEnd('/');
